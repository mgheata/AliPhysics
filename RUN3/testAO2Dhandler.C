...
void testAO2Dhandler()
{
   const char *anatype = "O2events";

 //  TGrid::Connect("alien:");

   AliAnalysisManager *mgr = new AliAnalysisManager("test_AO2D_handler");
   AliAO2DInputHandler *handler = new AliAO2DInputHandler();
   mgr->SetInputEventHandler(handler);
      
   auto task = new AliAnalysisTaskAO2D("AO2D_Pt");
   mgr->AddTask(task);
   mgr->ConnectInput(task, 0, mgr->GetCommonInputContainer());
   mgr->ConnectOutput(task, 1, mgr->CreateContainer("coutlist", TList::Class(), AliAnalysisManager::kOutputContainer, mgr->GetCommonFileName()));
