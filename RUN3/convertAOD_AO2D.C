...
TChain* CreateChain(const char *xmlfile, const char *type="AOD");
TChain *CreateLocalChain(const char *txtfile, const char *type, int nfiles);

void convertAOD_AO2D()
{
 ...
 //  TGrid::Connect("alien:");
   ...
   AliAODInputHandler *handler = AddAODHandler();
   ...
   AliAnalysisTaskAOD_AO2Dconverter* converter = AliAnalysisTaskAOD_AO2Dconverter::AddTask("");
   converter->SetMCMode();
   ...
   TChain *chain = CreateLocalChain("wn.txt", anatype, 10);
   if (!chain) return;
