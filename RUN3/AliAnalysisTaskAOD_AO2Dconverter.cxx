...
/* AliAnalysisTaskAOD_AO2Dconverter
 *
...
#include "AliAnalysisTaskAOD_AO2Dconverter.h"
...
ClassImp(AliAnalysisTaskAOD_AO2Dconverter);
...
AliAnalysisTaskAOD_AO2Dconverter::AliAnalysisTaskAOD_AO2Dconverter(const char* name)
...
AliAnalysisTaskAOD_AO2Dconverter::~AliAnalysisTaskAOD_AO2Dconverter()
{
  fOutputList->Delete();
  delete fOutputList;
  for (Int_t i = 0; i < kTrees; i++)
...
const TString AliAnalysisTaskAOD_AO2Dconverter::TreeName[kTrees] = { "O2collision", "DbgEventExtra", "O2track", "O2calo",  "O2calotrigger", "O2muon", "O2muoncluster", "O2zdc", "Run2v0", "O2fdd", "O2v0", "O2cascade", "O2tof", "O2mcparticle", "O2mccollision", "O2mctracklabel", "O2mccalolabel", "O2mccollisionlabel", "O2bc" };

const TString AliAnalysisTaskAOD_AO2Dconverter::TreeTitle[kTrees] = { "Collision tree", "Collision extra", "Barrel tracks", "Calorimeter cells", "Calorimeter triggers", "MUON tracks", "MUON clusters", "ZDC", "Run2 V0", "FDD", "V0s", "Cascades", "TOF hits", "Kinematics", "MC collisions", "MC track labels", "MC calo labels", "MC collision labels", "BC info" };

const TClass* AliAnalysisTaskAOD_AO2Dconverter::Generator[kGenerators] = { AliGenEventHeader::Class(), AliGenCocktailEventHeader::Class(), AliGenDPMjetEventHeader::Class(), AliGenEpos3EventHeader::Class(), AliGenEposEventHeader::Class(), AliGenEventHeaderTunedPbPb::Class(), AliGenGeVSimEventHeader::Class(), AliGenHepMCEventHeader::Class(), AliGenHerwigEventHeader::Class(), AliGenHijingEventHeader::Class(), AliGenPythiaEventHeader::Class(), AliGenToyEventHeader::Class() };

TTree* AliAnalysisTaskAOD_AO2Dconverter::CreateTree(TreeIndex t)
{
  fTree[t] = new TTree(TreeName[t], TreeTitle[t]);
  return fTree[t];
}

void AliAnalysisTaskAOD_AO2Dconverter::PostTree(TreeIndex t)
{
  if (!fTreeStatus[t])
    return;
...
void AliAnalysisTaskAOD_AO2Dconverter::FillTree(TreeIndex t)
{
  if (!fTreeStatus[t])
    return;
  fTree[t]->Fill();
...
void AliAnalysisTaskAOD_AO2Dconverter::UserCreateOutputObjects()
{
...
void AliAnalysisTaskAOD_AO2Dconverter::Prune()
{
...
void AliAnalysisTaskAOD_AO2Dconverter::UserExec(Option_t *)
{
  ...
  if (!fAOD) {
    ::Fatal("AliAnalysisTaskAOD_AO2Dconverter::UserExec", "Something is wrong with the event handler");
  }
  ...
  if (fUseEventCuts || fSkipPileup || fSkipTPCPileup) {
    skip_event = !fEventCuts.AcceptEvent(fAOD) && fUseEventCuts;
  }
  ...
  if (!pvtx) {
    ::Fatal("AliAnalysisTaskAOD_AO2Dconverter::UserExec", "Vertex not defined");
  }
  TString title=pvtx->GetTitle();
  
  // bypass vertex selection for muon UPC triggers with at least one muon track
  if (!(header->GetFiredTriggerClasses().Contains("CMUP") && fAOD->GetNumberOfMuonTracks()>0)) {
  ...
  bc.fRunNumber = fAOD->GetRunNumber();
  
  ULong64_t evtid = GetEventIdAsLong(fAOD->GetHeader());
  if(!evtid){
    evtid = (ULong64_t(fAOD->GetTimeStamp())<<32) + ULong64_t((fAOD->GetNumberOfTPCClusters()<<5)|(fAOD->GetNumberOfTPCTracks()));
  }
  bc.fGlobalBC = evtid;
  
  bc.fTriggerMask = fAOD->GetTriggerMask();
  TString firedClasses = fAOD->GetFiredTriggerClasses();
...
//      continue;
    ...
    track->GetXYZ(position);

    tracks.fCollisionsID = eventID;
    tracks.fTrackType = TrackTypeEnum::GlobalTrack;

    tracks.fX = AliMathBase::TruncateFloatFraction(position[0] /*track->GetX()*/, mTrackX);
    tracks.fAlpha = AliMathBase::TruncateFloatFraction(track->GetAlpha(), mTrackAlpha); // will rteturn 0, not stored in aod track

    tracks.fY = position[1]; /*track->GetY();*/ // no lossy compression
    tracks.fZ = position[2]; /*track->GetZ();*/
    tracks.fSnp = AliMathBase::TruncateFloatFraction(track->Py()/track->Pt() /*track->GetSnp()*/, mtrackSnp);
    tracks.fTgl = AliMathBase::TruncateFloatFraction(track->GetTgl(), mTrackTgl); // returns 0, should be derived
    tracks.fSigned1Pt = AliMathBase::TruncateFloatFraction(track->OneOverPt() /*track->GetSigned1Pt()*/, mTrack1Pt);

    // Modified covariance matrix
    // First sigmas on the diagonal
    tracks.fSigmaY = AliMathBase::TruncateFloatFraction(TMath::Sqrt(cv[0] /*track->GetSigmaY2()*/), mTrackCovDiag);
    tracks.fSigmaZ = AliMathBase::TruncateFloatFraction(TMath::Sqrt(cv[2] /*track->GetSigmaZ2()*/), mTrackCovDiag);
    tracks.fSigmaSnp = AliMathBase::TruncateFloatFraction(TMath::Sqrt(cv[5] /*track->GetSigmaSnp2()*/), mTrackCovDiag);
    tracks.fSigmaTgl = AliMathBase::TruncateFloatFraction(TMath::Sqrt(cv[9] /*track->GetSigmaTgl2()*/), mTrackCovDiag);
    tracks.fSigma1Pt = AliMathBase::TruncateFloatFraction(TMath::Sqrt(cv[14] /*track->GetSigma1Pt2()*/), mTrackCovDiag);
    //
    tracks.fRhoZY = (Char_t)(128.*cv[1] /*track->GetSigmaZY()*//tracks.fSigmaZ/tracks.fSigmaY);
    tracks.fRhoSnpY = (Char_t)(128.*cv[3] /*track->GetSigmaSnpY()*//tracks.fSigmaSnp/tracks.fSigmaY);
    tracks.fRhoSnpZ = (Char_t)(128.*cv[4] /*track->GetSigmaSnpZ()*//tracks.fSigmaSnp/tracks.fSigmaZ);
    tracks.fRhoTglY = (Char_t)(128.*cv[6] /*track->GetSigmaTglY()*//tracks.fSigmaTgl/tracks.fSigmaY);
    tracks.fRhoTglZ = (Char_t)(128.*cv[7] /*track->GetSigmaTglZ()*//tracks.fSigmaTgl/tracks.fSigmaZ);
    tracks.fRhoTglSnp = (Char_t)(128.*cv[8] /*track->GetSigmaTglSnp()*//tracks.fSigmaTgl/tracks.fSigmaSnp);
    tracks.fRho1PtY = (Char_t)(128.*cv[10] /*track->GetSigma1PtY()*//tracks.fSigma1Pt/tracks.fSigmaY);
    tracks.fRho1PtZ = (Char_t)(128.*cv[11] /*track->GetSigma1PtZ()*//tracks.fSigma1Pt/tracks.fSigmaZ);
    tracks.fRho1PtSnp = (Char_t)(128.*cv[12] /*track->GetSigma1PtSnp()*//tracks.fSigma1Pt/tracks.fSigmaSnp);
    tracks.fRho1PtTgl = (Char_t)(128.*cv[13] /*track->GetSigma1PtTgl()*//tracks.fSigma1Pt/tracks.fSigmaTgl);
    ...
    tracks.fTPCinnerP = AliMathBase::TruncateFloatFraction(track->PAtDCA(), mTrack1Pt); //  To be checked
    ...
    if (fTaskMode == kMC) {
      ...
      UChar_t itsMask = track->GetITSSharedClusterMap() /*GetITSSharedMap()*/ & 0x1F; // Normally only bits 0-5 are set in Run1/2
  ...
  // fAOD->GetMultiplicity() returns AliAODTrackelts which derived from AliVMultiplicity

  AliVMultiplicity *mlt = fAOD->GetMultiplicity();
  ...
  for (Int_t itr = Ntracklets; itr--;) {
    ...
    // *** CalcDist does not exist in AliAODTracklets ***
    dist   = 0  /*mlt->CalcDist(itr)*/;
  ...
  AliEMCALGeometry *geo = AliEMCALGeometry::GetInstanceFromRunNumber(fAOD->GetRunNumber()); // Needed for EMCAL trigger mapping
  AliAODCaloTrigger *calotriggers = fAOD->GetCaloTrigger("EMCAL");
  calotriggers->Reset();
  ...
  //---------------------------------------------------------------------------
  // Muon tracks
  // **** Specific info from AliESDMuonTrack is not copied to AOD track
  ...
  */
  ...
  //---------------------------------------------------------------------------
  // ZDC
  AliAODZDC* aodzdc = fAOD->GetZDCData() /*GetESDZDC()*/;
  zdc.fBCsID        = eventID;
  // ZEM
  ...
  // ZDC TDC
  Bool_t isHitFlagFilled = fAOD->GetRunNumber()>=208502;
  Bool_t isZNAhit  = isHitFlagFilled ? aodzdc->IsZNAfired() : 1;
  Bool_t isZNChit  = isHitFlagFilled ? aodzdc->IsZNCfired() : 1;
  Bool_t isZPAhit  = isHitFlagFilled ? aodzdc->IsZPAfired() : 1;
  Bool_t isZPChit  = isHitFlagFilled ? aodzdc->IsZPCfired() : 1;
  //Bool_t isZEM1hit = isHitFlagFilled ? aodzdc->IsZEM1fired() : 1; // not used in AOD
  //Bool_t isZEM2hit = isHitFlagFilled ? aodzdc->IsZEM2fired() : 1;
  ...
  for (Int_t i=0;i<4;i++) {
    Float_t tZNA  = isZNAhit  ? aodzdc->GetZNATDCm(i) /*esdzdc->GetZDCTDCCorrected(esdzdc->GetZNATDCChannel(),i)*/  : 999.f;
    Float_t tZNC  = isZNChit  ? aodzdc->GetZNCTDCm(i) /*esdzdc->GetZDCTDCCorrected(esdzdc->GetZNCTDCChannel(),i)*/  : 999.f;
    Float_t tZPA  = isZPAhit  ? aodzdc->GetZPATDCm(i) /*esdzdc->GetZDCTDCCorrected(esdzdc->GetZPATDCChannel(),i)*/  : 999.f;
    Float_t tZPC  = isZPChit  ? aodzdc->GetZPCTDCm(i) /*esdzdc->GetZDCTDCCorrected(esdzdc->GetZPCTDCChannel(),i)*/  : 999.f;
    // ZEM1/2 info not stored in AOD
  ...
  //---------------------------------------------------------------------------
  // VZERO
  AliAODVZERO* vz = fAOD->GetVZEROData();
  vzero.fBCsID  = eventID;
  for (Int_t ich=0; ich<64; ++ich) {
    // No info in AOD
    vzero.fAdc[ich] = 0 /*vz->GetAdc(ich)*/;
    vzero.fTime[ich] = 0 /*vz->GetTime(ich)*/;
    vzero.fWidth[ich] = 0 /*vz->GetWidth(ich)*/;
    vzero.fBBFlag = 0u;
    vzero.fBGFlag = 0u;
    ULong64_t mask = 1u;
  ...
  for (Int_t iv0=0; iv0<nv0; ++iv0) {
    ...
    if (v0 && !v0->GetOnFlyStatus()) {
      Int_t pidx = v0->GetPosID() /*GetPindex()*/; // to check
      Int_t nidx = v0->GetNegID() /*GetNindex()*/;
  ...
  if (nv0>0) {
    ...
    Int_t ncas = fAOD->GetNumberOfCascades();
	...
	if (sortedPosNeg[v0idx] == currV0) {
	  ...
	  //cascs.fTracksID = cas->GetBindex() + fOffsetTrackID; // GetBindex ???
	  FillTree(kCascades);
...
void AliAnalysisTaskAOD_AO2Dconverter::Terminate(Option_t *)
{
  // terminate
  // called at the END of the analysis (when all events are processed)
}

AliAnalysisTaskAOD_AO2Dconverter *AliAnalysisTaskAOD_AO2Dconverter::AddTask(TString suffix)
{
  ...
  AliAnalysisTaskAOD_AO2Dconverter *task = new AliAnalysisTaskAOD_AO2Dconverter((TString("AO2D") + suffix).Data());
  if (!task)
    return nullptr;
